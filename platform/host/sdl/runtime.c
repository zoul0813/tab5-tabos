...
static char *window_state_path(void)
{
    char *pref_path = SDL_GetPrefPath(
        TABOS_HOST_PREFERENCES_ORGANIZATION, TABOS_HOST_PREFERENCES_APPLICATION);
    if (pref_path == NULL) return NULL;
    static const char filename[] = TABOS_HOST_WINDOW_STATE_FILENAME;
    const size_t path_size = strlen(pref_path) + sizeof(filename);
    char *path = malloc(path_size);
    if (path != NULL) (void)snprintf(path, path_size, "%s%s", pref_path, filename);
    SDL_free(pref_path);
    return path;
}
...
static bool position_is_visible(int x, int y)
{
    ...
    for (int index = 0; index < display_count; ++index) {
        SDL_Rect bounds;
        if (!SDL_GetDisplayUsableBounds(displays[index], &bounds)) continue;
        const long long display_right = (long long)bounds.x + bounds.w;
        const long long display_bottom = (long long)bounds.y + bounds.h;
        if (window_left < display_right && window_right > bounds.x &&
            window_top < display_bottom && window_bottom > bounds.y) {
            visible = true;
            break;
        }
    }
