...
{
#if TABOS_ENABLE_PIE
    if (destination == NULL || color == NULL || count < PIE_FILL_MIN_PIXELS ||
        !aligned16(destination)) return false;
...
{
#if TABOS_ENABLE_PIE
    if (destination == NULL || source == NULL || count < PIE_COPY_MIN_PIXELS ||
        !aligned16(destination) || !aligned16(source)) return false;
...
static const pie_diagnostic_size_t diagnostic_sizes[] = {
    {8U, "glyph-run"}, {16U, "glyph-run"}, {32U, "span"}, {64U, "span"},
    {256U, "span"}, {384U, "terminal-cell"}, {1024U, "span"},
    {4096U, "small-sprite"}, {65536U, "large-region"},
};

static void benchmark_size(const char *placement, uint16_t *source,
                           uint16_t *destination, const pie_diagnostic_size_t *test)
{
    ...
    for (unsigned int round = 0U; round < PIE_DIAGNOSTIC_WARMUP_ROUNDS; ++round) {
        ...
        (void)esp32p4_pie_fill16(destination, pixels,
                                &(const uint16_t){UINT16_C(0x5aa5)});
    ...
    ESP_LOGI(TAG, "%s fill %u px (%s): scalar=%" PRId64 " us PIE=%" PRId64
             " us %.2fx %.1f MiB/s hash=%08" PRIx32 " %s",
             ...
             pie_fill_us > 0 ? mib * 1000000.0 / (double)pie_fill_us : 0.0,
             fill_hash, fill_hash == expected_fill_hash ? "MATCH" : "MISMATCH");
    ESP_LOGI(TAG, "%s copy %u px (%s): libc=%" PRId64 " us PIE=%" PRId64
             " us %.2fx %.1f MiB/s hash=%08" PRIx32 " %s",
             ...
             pie_copy_us > 0 ? mib * 1000000.0 / (double)pie_copy_us : 0.0,
             output_copy_hash, output_copy_hash == copy_hash ? "MATCH" : "MISMATCH");
}
...
static void benchmark_memory(const char *placement, uint32_t caps)
{
    ...
    if (source == NULL || destination == NULL) {
        ...
        free(source); free(destination); return;
    }
    for (size_t index = 0U; index < sizeof(diagnostic_sizes) / sizeof(diagnostic_sizes[0]);
         ++index) {
...
{
#if TABOS_ENABLE_PIE_DIAGNOSTICS
    ESP_LOGI(TAG, "PIE diagnostics: aligned size sweep; about %u pixels per sample",
             PIE_DIAGNOSTIC_TARGET_PIXELS);
