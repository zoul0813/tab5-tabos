...
static bool initialize_ppa(void)
{
    ...
    ppa_scratch              = heap_caps_aligned_calloc(64U, pixel_count, sizeof(*ppa_scratch), MALLOC_CAP_SPIRAM);
    if (ppa_scratch == NULL) {
        return false;
    }
    ...
    if (result == ESP_OK) {
        ...
        if (result == ESP_OK) {
            result = ppa_register_client(&client_config, &ppa_fill_client);
        }
        if (result == ESP_OK) {
        ...
        if (result == ESP_OK) {
            result = ppa_register_client(&client_config, &ppa_blend_client);
        }
        if (result == ESP_OK) {
...
{
    const size_t buffer_size = framebuffer->width * framebuffer->height * sizeof(*framebuffer->pixels);
...
static tab5_panel_type_t detect_panel(void)
{
    ...
    const esp_err_t read_result =
        esp_lcd_panel_io_rx_param(touch_io, 0x0000, &firmware_version, sizeof(firmware_version));
...
static esp_err_t create_st7121(void)
{
    ...
    esp_lcd_dpi_panel_config_t dpi_config      = ST7121_1280_720_PANEL_60HZ_DPI_CONFIG(LCD_COLOR_PIXEL_FORMAT_RGB565);
...
static bool wait_for_ppa(esp_err_t result)
{
    ...
    if (ppa_ready) {
        ESP_LOGW(TAG, "PPA operation failed; switching to software graphics");
    }
    ppa_ready = false;
    return false;
}
...
static void native_fill_cpu(int32_t left, int32_t top, int32_t right, int32_t bottom, platform_pixel_t color)
{
...
static bool native_color_keyed(platform_pixel_t color, platform_pixel_t low, platform_pixel_t high)
{
    return ((color >> 11U) & 0x1fU) >= ((low >> 11U) & 0x1fU) && ((color >> 11U) & 0x1fU) <= ((high >> 11U) & 0x1fU) &&
           ((color >> 5U) & 0x3fU) >= ((low >> 5U) & 0x3fU) && ((color >> 5U) & 0x3fU) <= ((high >> 5U) & 0x3fU) &&
           (color & 0x1fU) >= (low & 0x1fU) && (color & 0x1fU) <= (high & 0x1fU);
}

static platform_pixel_t native_blend(platform_pixel_t source, platform_pixel_t destination, uint8_t opacity)
{
...
{
    if (options == NULL || options->pixels == NULL || options->source.x < 0 || options->source.y < 0 ||
    ...
    for (uint32_t dy = 0U; dy < options->destination.height; ++dy) {
        ...
        for (uint32_t dx = 0U; dx < options->destination.width; ++dx) {
            ...
            const platform_pixel_t source  = options->pixels[(size_t) sy * options->bitmap_width + sx];
...
static bool native_blit_ppa(const tabos_graphics_blit_options_t* options)
{
    if (!ppa_ready || options == NULL || options->pixels == NULL || options->source.x < 0 || options->source.y < 0 ||
        options->source.width == 0U || options->source.height == 0U || options->destination.width == 0U ||
        options->destination.height == 0U || options->destination.x < 0 || options->destination.y < 0 ||
        (uint64_t) (uint32_t) options->source.x + options->source.width > options->bitmap_width ||
        (uint64_t) (uint32_t) options->source.y + options->source.height > options->bitmap_height ||
    ...
    const uint32_t native_y = TABOS_DISPLAY_WIDTH - ((uint32_t) options->destination.x + options->destination.width);
    while (xSemaphoreTake(ppa_done, 0) == pdTRUE) {}
    const ppa_srm_oper_config_t config = {
        .in =
            {
                 .buffer         = options->pixels,
                 .pic_w          = options->bitmap_width,
                 .pic_h          = options->bitmap_height,
                 .block_w        = options->source.width,
    ...
    if (!wait_for_ppa(ppa_do_scale_rotate_mirror(ppa_srm_client, &config))) {
        return false;
    }
    direct_frame_dirty = true;
    return true;
                            ...
                            platform_pixel_t color)
{
    ...
    const ppa_fill_oper_config_t config = {
        .out =
            {
                  .buffer         = framebuffer->pixels,
                  .buffer_size    = (uint32_t) (framebuffer->width * framebuffer->height * sizeof(*framebuffer->pixels)),
                  .pic_w          = (uint32_t) framebuffer->width,
                  .pic_h          = (uint32_t) framebuffer->height,
...
bool platform_graphics_blit(platform_framebuffer_t* framebuffer, const tabos_graphics_blit_options_t* options)
{
    ...
    if ((options->opacity != 255U || options->color_key_enabled) && options->rotation == TABOS_GRAPHICS_ROTATE_0 &&
        !options->mirror_x && !options->mirror_y && options->source.width == options->destination.width &&
        options->source.height == options->destination.height) {
        while (xSemaphoreTake(ppa_done, 0) == pdTRUE) {}
        const uint32_t buffer_size =
            (uint32_t) (framebuffer->width * framebuffer->height * sizeof(*framebuffer->pixels));
        ...
        if (!wait_for_ppa(ppa_do_blend(ppa_blend_client, &blend))) {
            return false;
        }
        for (uint32_t row = 0U; row < options->destination.height; ++row) {
            const size_t offset = ((size_t) (uint32_t) options->destination.y + row) * framebuffer->stride_pixels +
                                  (uint32_t) options->destination.x;
...
bool platform_display_init(platform_framebuffer_t* framebuffer)
{
    ...
    const size_t pixel_count = (size_t) TABOS_DISPLAY_WIDTH * TABOS_DISPLAY_HEIGHT;
    logical_pixels           = heap_caps_aligned_calloc(64U, pixel_count, sizeof(*logical_pixels), MALLOC_CAP_SPIRAM);
    ...
    if (panel_type == TAB5_PANEL_ST7121) {
        ...
        const bsp_display_config_t config = {
            .dsi_bus =
                {
                          .phy_clk_src        = 0,
                          .lane_bit_rate_mbps = panel_type == TAB5_PANEL_ILI9881C ? BSP_LCD_MIPI_DSI_LANE_BITRATE_MBPS :
                                                                              TAB5_DSI_LANE_BITRATE_MBPS, },
        };
    ...
    const esp_err_t framebuffer_result = esp_lcd_dpi_panel_get_frame_buffer(
        display_handles.panel, 2, (void**) &native_framebuffers[0], (void**) &native_framebuffers[1]);
    if (framebuffer_result != ESP_OK || native_framebuffers[0] == NULL || native_framebuffers[1] == NULL) {
    ...
    const esp_err_t callback_result =
        esp_lcd_dpi_panel_register_event_callbacks(display_handles.panel, &callbacks, vsync_done);
    if (callback_result != ESP_OK) {
        ESP_LOGE(TAG, "Could not register display VSYNC callback: %s", esp_err_to_name(callback_result));
        platform_display_shutdown();
        return false;
    }
