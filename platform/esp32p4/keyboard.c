...
static uint8_t current_modifiers(void)
{
    ...
    if (pressed_keys[KEYBOARD_CONTROL_INDEX]) modifiers |= TABOS_MODIFIER_CONTROL;
    if (pressed_keys[KEYBOARD_SHIFT_INDEX]) modifiers |= TABOS_MODIFIER_SHIFT;
    if (pressed_keys[KEYBOARD_ALT_INDEX]) modifiers |= TABOS_MODIFIER_ALT;
    if (pressed_keys[KEYBOARD_SYM_INDEX]) modifiers |= TABOS_MODIFIER_SYM;
    return modifiers;
}
...
{
    if (index == KEYBOARD_SYM_INDEX) *key = TABOS_KEY_SYM;
    else if (index == KEYBOARD_SHIFT_INDEX) *key = TABOS_KEY_SHIFT;
    else if (index == KEYBOARD_CONTROL_INDEX) *key = TABOS_KEY_CTRL;
    else if (index == KEYBOARD_ALT_INDEX) *key = TABOS_KEY_ALT;
    else return false;
    return true;
}
...
static void submit_matrix_event(uint8_t report)
{
    ...
    if (row >= KEYBOARD_ROWS || column >= KEYBOARD_COLUMNS) return;
    const size_t index = KEYBOARD_INDEX(row, column);
    if (pressed_keys[index] == pressed) return;
    pressed_keys[index] = pressed;

    tabos_key_t modifier;
    if (modifier_key(index, &modifier)) {
        if (index == KEYBOARD_SYM_INDEX) {
            if (pressed) sym_used = false;
            else if (!sym_used) sym_latched = true;
        } else if (index == KEYBOARD_SHIFT_INDEX) {
            if (pressed) shift_used = false;
            else if (!shift_used) shift_latched = true;
        }
    ...
    if (pressed) {
        ...
        if (cooked_sym) cooked_modifiers |= TABOS_MODIFIER_SYM;
        ...
        if (pressed_keys[KEYBOARD_SYM_INDEX]) sym_used = true;
        if (pressed_keys[KEYBOARD_SHIFT_INDEX]) shift_used = true;
        sym_latched = false;
        shift_latched = false;
    } else {
    ...
    if (usage == TABOS_KEY_DELETE) delete_held = pressed;
}
...
static void capture_boot_delete(void)
{
    ...
    for (uint8_t index = 0U; index < count; ++index) {
        ...
        if (keyboard_read(KEYBOARD_REG_KEY_EVENT, &report, sizeof(report)) != ESP_OK || report == 0xffU) break;
