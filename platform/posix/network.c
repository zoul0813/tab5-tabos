...
static native_cancel_t network_cancellation;
void platform_network_operations_cancel(const void* owner)
{
    native_cancel_request(&network_cancellation, owner);
}
#else
void platform_network_operations_cancel(const void* owner)
{
                                                               ...
                                                               platform_network_echo_result_t* result)
{
    ...
    for (;;) {
#if defined(ESP_PLATFORM)
        if (native_cancel_pending(&network_cancellation)) {
            close_socket(descriptor);
            return PLATFORM_NETWORK_OPERATION_IO;
        }
#endif
        ...
        const ssize_t received =
            recvfrom(descriptor, packet, sizeof(packet), 0, (struct sockaddr*) &source, &source_size);
        if (received < 0) {
                                                          ...
                                                          network_worker_response_t* response)
{
    ...
    while (xSemaphoreTake(worker_mutex, 1U) != pdTRUE) {
        if (platform_riscv32_current_cancelled()) {
            return PLATFORM_NETWORK_OPERATION_IO;
        }
    }
    native_cancel_begin(&network_cancellation, platform_riscv32_current_user_data());
    if (platform_riscv32_current_cancelled()) {
