...
static bool translate_path(char drive_letter, const char* path, char output[TABOS_FS_PATH_MAX], const char** drive_root)
{
    storage_drive_t* drive = find_drive(drive_letter);
    if (drive == NULL || path == NULL || path[0] != '/') {
        return false;
    }
    const size_t root_length = strlen(drive->root);
    const size_t path_length = strlen(path);
    if (root_length + path_length >= TABOS_FS_PATH_MAX) {
        return false;
    }
...
static int reject_symlink_components(const char* root, const char* translated, bool allow_missing_final)
{
#ifdef ESP_PLATFORM
    (void) root;
    (void) translated;
    ...
    for (size_t index = root_length + 1U; index <= length; ++index) {
        ...
        if (lstat(partial, &status) != 0) {
            ...
            partial[index]  = saved;
            if (allow_missing_final && saved == '\0' && error == ENOENT) {
                return 0;
            }
...
{
    if (storage_drive_count > 0U) {
        return true;
    }
    memset(storage_drives, 0, sizeof(storage_drives));
    const size_t count = storage_backend_drive_count();
    for (size_t index = 0U; index < count && storage_drive_count < PLATFORM_STORAGE_MAX_DRIVES; ++index) {
        storage_drive_t drive = {.name = "Filesystem"};
        if (!storage_backend_mount(index, &drive.letter, drive.root, sizeof(drive.root), &drive.removable,
                                   &drive.name)) {
            continue;
        }
...
{
    if (info == NULL || index >= storage_drive_count) {
        return false;
    }
...
{
    if (file == NULL || (flags & TABOS_O_ACCMODE) > TABOS_O_RDWR) {
        return TABOS_EINVAL;
    }
    char translated[TABOS_FS_PATH_MAX];
    const char* root = NULL;
    if (!translate_path(drive, path, translated, &root)) {
        return TABOS_ENODEV;
    }
    ...
    if ((flags & TABOS_O_ACCMODE) == TABOS_O_WRONLY) {
        native_flags = O_WRONLY;
    }
    if ((flags & TABOS_O_ACCMODE) == TABOS_O_RDWR) {
        native_flags = O_RDWR;
    }
    if ((flags & TABOS_O_CREAT) != 0) {
        native_flags |= O_CREAT;
    }
    if ((flags & TABOS_O_EXCL) != 0) {
        native_flags |= O_EXCL;
    }
    if ((flags & TABOS_O_TRUNC) != 0) {
        native_flags |= O_TRUNC;
    }
    if ((flags & TABOS_O_APPEND) != 0) {
...
{
    if (position == NULL || (whence != TABOS_SEEK_SET && whence != TABOS_SEEK_CUR && whence != TABOS_SEEK_END)) {
        return TABOS_EINVAL;
    }
    const int native_whence = whence == TABOS_SEEK_SET ? SEEK_SET : (whence == TABOS_SEEK_CUR ? SEEK_CUR : SEEK_END);
...
int platform_storage_stat(char drive, const char* path, tabos_stat_t* status)
{
    ...
    if (stat(translated, &native_status) != 0) {
        return map_error(errno);
    }
...
int platform_storage_fstat(platform_file_t file, tabos_stat_t* status)
{
    ...
    struct stat native_status;
    if (fstat((int) file, &native_status) != 0) {
        return map_error(errno);
    }
    map_status(&native_status, status);
    return 0;
...
int platform_storage_rename(char drive, const char* old_path, const char* new_path)
{
    ...
    if (error == 0) {
        error = reject_symlink_components(root, new_translated, true);
    }
    if (error != 0) {
        return error;
    }
...
int platform_storage_readdir(platform_dir_t directory, tabos_dirent_t* entry, bool* end)
{
    ...
    if (name_length > TABOS_FS_NAME_MAX) {
        return TABOS_ENAMETOOLONG;
    }
