...
void raster_fill_span(platform_pixel_t* destination, size_t count, platform_pixel_t color)
{
    ...
    if (platform_raster_fill_span(destination, count, color)) {
        return;
    }
...
void raster_copy_span(platform_pixel_t* destination, const platform_pixel_t* source, size_t count)
{
    if (destination == NULL || source == NULL || count == 0U || destination == source) {
        return;
    }
    ...
    if ((destination_begin + bytes <= source_begin || source_begin + bytes <= destination_begin) &&
        platform_raster_copy_span(destination, source, count)) {
        return;
    }
...
bool raster_blit(platform_framebuffer_t* framebuffer, const tabos_graphics_blit_options_t* options)
{
    if (framebuffer == NULL || framebuffer->pixels == NULL || options == NULL || options->pixels == NULL ||
        options->bitmap_width == 0U || options->bitmap_height == 0U || options->source.width == 0U ||
    ...
    if (options->rotation == TABOS_GRAPHICS_ROTATE_0 && !options->mirror_x && !options->mirror_y &&
        ...
        for (uint32_t row = 0U; row < options->destination.height; ++row) {
            ...
            if (output_x >= (int64_t) framebuffer->width) {
                continue;
            }
            ...
            platform_pixel_t* destination =
                framebuffer->pixels + (size_t) output_y * framebuffer->stride_pixels + (size_t) output_x;
            const platform_pixel_t* source =
                options->pixels + ((size_t) (uint32_t) options->source.y + row) * options->bitmap_width + source_x;
            ...
            uint32_t column = 0U;
            while (column < count) {
                while (column < count && keyed(source[column], options->color_key_low, options->color_key_high)) {
                    ++column;
                }
