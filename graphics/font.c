                    ...
                    platform_pixel_t foreground, platform_pixel_t background)
{
    ...
    for (unsigned int row = 0; row < FONT_GLYPH_HEIGHT; ++row) {
        for (unsigned int sy = 0; sy < scale; ++sy) {
            ...
            if (output_y < 0 || (size_t) output_y >= framebuffer->height) {
                continue;
            }
            ...
            while (column < FONT_GLYPH_WIDTH) {
                ...
                while (column + run < FONT_GLYPH_WIDTH) {
                    ...
                    const platform_pixel_t next_color = (rows[next_byte] & next_mask) != 0U ? foreground : background;
                    if (next_color != color) {
                        break;
                    }
                    ++run;
                ...
                if ((size_t) output_x < framebuffer->width) {
                    ...
                    raster_fill_span(framebuffer->pixels + (size_t) output_y * framebuffer->stride_pixels +
                                         (size_t) output_x,
                                     count, color);
                }
                column += run;
