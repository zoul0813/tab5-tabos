...
| C6 power / hosted transport (`platform/esp32p4/runtime.c`, `esp_hosted`) | C6 powered at platform boot. SDIO 4-bit 40 MHz GPIO8–13, reset15; driver IRQ/DMA. `tabos_wifi_start` does asynchronous reset/firmware negotiation and staged Wi-Fi init. Network shutdown cooperatively cancels and joins startup, then unwinds owned Wi-Fi resources. | Blocking: disconnect alone does not quiesce SDIO/C6; hosted deinit/reinit must be validated with preserved config and reconnect intent. |
...
Resolved audit prerequisites now provide native task quiescence, Wi-Fi partial-init
unwind, and recoverable FAT replacement. Historical shutdown success or a passing host
suite does not prove full suspend safety. Refresh this inventory as lifecycle behavior
changes, especially process lifetimes.
