...
Current process foundation provides a fixed-capacity process table, stable process IDs, parent/state metadata, and one foreground root process. Selected console and filesystem diagnostics remain persistent as temporary PID 0 programs after reporting completion. Selecting independent `hello_elf` as PID 0 intentionally exercises root liveness: return or explicit exit places PID 0 in kernel-panic state without unloading its process record. Parent/child nesting and managed Tab5 application tasks remain next work.
