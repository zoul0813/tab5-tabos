...
enum {
    APPLICATION_REGISTRY_CAPACITY = 16
};
...
static bool descriptor_valid(const tabos_app_descriptor_t* descriptor)
{
    ...
    return descriptor != NULL && descriptor->abi_version == TABOS_APPLICATION_ABI_VERSION && descriptor->name != NULL &&
           descriptor->name[0] != '\0' && descriptor->version != NULL && descriptor->version[0] != '\0' &&
