...
static const char *key_name(tabos_key_t key)
{
    ...
    if (key >= TABOS_KEY_A && key <= TABOS_KEY_Z) return letters[key - TABOS_KEY_A];
    if (key >= TABOS_KEY_1 && key <= TABOS_KEY_0) return digits[key - TABOS_KEY_1];
    switch (key) {
...
void tab_input_diagnostic_log(const tabos_input_event_t *event)
{
    ...
    if (event->type == TABOS_INPUT_TEXT) {
        ...
        bool first = true;
        if ((event->modifiers & TABOS_MODIFIER_CONTROL) != 0U) append_modifier(line, sizeof(line), "CTRL", &first);
