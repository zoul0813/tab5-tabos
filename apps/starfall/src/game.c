...
enum { PLAYER_WIDTH = 20, PLAYER_HEIGHT = 16, PLAYER_SPEED = 4,
       ENEMY_WIDTH = 18, ENEMY_HEIGHT = 14, SHOT_WIDTH = 3, SHOT_HEIGHT = 8 };
...
{
    if (game->mode == STARFALL_PLAYING) game->mode = STARFALL_PAUSED;
    else if (game->mode == STARFALL_PAUSED) game->mode = STARFALL_PLAYING;
}
...
{
    if (!game->input.fire || game->tick < game->next_fire_tick) return;
    for (unsigned int index = 0U; index < STARFALL_SHOT_COUNT; ++index) {
        if (game->shots[index].active) continue;
        game->shots[index] = (starfall_shot_t){
            .x = (int16_t)(game->player_x + PLAYER_WIDTH / 2 - 1),
            .y = (int16_t)(game->player_y - SHOT_HEIGHT), .active = true,
        };
...
{
    if (game->tick < game->invulnerable_until) return;
    if (game->lives > 0U) --game->lives;
    game->damage_flash_until = game->tick + 10U;
    game->invulnerable_until = game->tick + 90U;
    emit_particles(game, game->player_x + PLAYER_WIDTH / 2,
                   game->player_y + PLAYER_HEIGHT / 2);
    if (game->lives == 0U) {
...
void starfall_game_update(starfall_game_t *game)
{
    ...
    if (game->mode != STARFALL_PLAYING) return;
    ++game->tick;

    game->player_x += (game->input.right ? PLAYER_SPEED : 0) -
                      (game->input.left ? PLAYER_SPEED : 0);
    ...
    for (unsigned int index = 0U; index < STARFALL_SHOT_COUNT; ++index) {
        ...
        if (shot->y < 22) shot->active = false;
    }
    for (unsigned int index = 0U; index < STARFALL_ENEMY_COUNT; ++index) {
        starfall_enemy_t *enemy = &game->enemies[index];
        if (!enemy->active) continue;
        ...
        for (unsigned int shot_index = 0U; shot_index < STARFALL_SHOT_COUNT; ++shot_index) {
            ...
            if (!shot->active || !overlaps(shot->x, shot->y, SHOT_WIDTH, SHOT_HEIGHT,
            ...
            if (game->score > game->high_score) game->high_score = game->score;
            emit_particles(game, enemy->x + ENEMY_WIDTH / 2,
                           enemy->y + ENEMY_HEIGHT / 2);
            break;
        }
        if (enemy->active && overlaps(game->player_x, game->player_y,
                PLAYER_WIDTH, PLAYER_HEIGHT, enemy->x, enemy->y,
    ...
    for (unsigned int index = 0U; index < STARFALL_PARTICLE_COUNT; ++index) {
        ...
        if (--particle->life == 0U) particle->active = false;
    }
}
