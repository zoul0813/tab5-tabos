...
static void draw_text(tabos_graphics_t* graphics, int x, int y, const char* text, unsigned int scale,
                      tabos_color_t color)
{
    while (*text != '\0') {
...
static void centered(tabos_graphics_t* graphics, int y, const char* text, unsigned int scale, tabos_color_t color)
{
...
{
    if (game->tick < game->invulnerable_until && ((game->tick / 4U) & 1U) != 0U) {
        return;
    }
...
{
    const tabos_color_t color = enemy->kind == 0U ? COLOR_GREEN : enemy->kind == 1U ? COLOR_YELLOW : COLOR_RED;
...
{
    if (tabos_graphics_clear(graphics, COLOR_SPACE) != 0) {
        return -1;
    }
    for (unsigned int index = 0U; index < STARFALL_STAR_COUNT; ++index) {
        ...
        const tabos_color_t color   = star->speed == 1U ? TABOS_RGB565(48, 64, 96) :
                                      star->speed == 2U ? TABOS_RGB565(112, 144, 192) :
                                                          COLOR_WHITE;
        (void) tabos_graphics_fill_rect(graphics, star->x, star->y, star->speed == 3U ? 2U : 1U, star->speed, color);
    }
    (void) tabos_graphics_fill_rect(graphics, 0, 21, STARFALL_WIDTH, 1U, TABOS_RGB565(24, 72, 112));
    char hud[72];
    (void) snprintf(hud, sizeof(hud), "SCORE %06lu  HI %06lu  LIVES %u  WAVE %lu", (unsigned long) game->score,
                    (unsigned long) game->high_score, game->lives, (unsigned long) game->wave);
    ...
    if (game->tick < game->damage_flash_until) {
        (void) tabos_graphics_rect(graphics, 2, 23, STARFALL_WIDTH - 4U, STARFALL_HEIGHT - 25U, COLOR_RED);
    }
