...
#define CHECK(operation, expression)      \
    do {                                  \
    ...
    descriptor = tabos_fs_open(test_file, TABOS_O_CREAT | TABOS_O_RDWR | TABOS_O_TRUNC, 0644U);
    CHECK("Create file", descriptor >= 0);
    CHECK("Write payload", tabos_fs_write(descriptor, payload, sizeof(payload)) == (tabos_ssize_t) sizeof(payload));
    CHECK("Seek to start", tabos_fs_seek(descriptor, 0, TABOS_SEEK_SET) == 0);

    char buffer[sizeof(payload)] = {0};
    CHECK("Read payload", tabos_fs_read(descriptor, buffer, sizeof(buffer)) == (tabos_ssize_t) sizeof(buffer));
    CHECK("Verify payload", memcmp(buffer, payload, sizeof(payload)) == 0);

    tabos_stat_t status;
    CHECK("Inspect open file", tabos_fs_fstat(descriptor, &status) == 0 && (status.mode & TABOS_S_IFREG) != 0U &&
                                   status.size == sizeof(payload));
    ...
    CHECK("Read reopened file", tabos_fs_read(descriptor, buffer, sizeof(buffer)) == (tabos_ssize_t) sizeof(buffer) &&
                                    memcmp(buffer, payload, sizeof(payload)) == 0);
    CHECK("Close reopened file", tabos_fs_close(descriptor) == 0);
    descriptor = -1;

    CHECK("Inspect file path", tabos_fs_stat(test_file, &status) == 0 && status.size == sizeof(payload));
    ...
    while ((read_result = tabos_fs_readdir(directory, &entry)) > 0) {
        if (strcmp(entry.name, "renamed.txt") == 0) {
            found = true;
        }
    }
...
cleanup:
    if (descriptor >= 0) {
        (void) tabos_fs_close(descriptor);
    }
    if (directory >= 0) {
        (void) tabos_fs_closedir(directory);
    }
    if (passed) {
        remove_previous_test();
    }
...
static bool diagnostic_entry(tabos_app_context_t* context)
{
    ...
    (void) tabos_console_write(console, passed ? "Filesystem diagnostic passed\n" : "Filesystem diagnostic failed\n");
    application_report_diagnostic_result(context, passed ? 0 : 1);
    return true;
}
...
const tabos_app_descriptor_t filesystem_diagnostic_app = {
