...
static loader_elf_image_t loaded_image;
static platform_riscv32_context_t *execution_context;
...
static bool elf_entry(tabos_app_context_t *context)
{
    ...
    if (result != LOADER_ELF_OK) {
        ...
        (void)tabos_console_write_line(active_console, loader_elf_result_name(result));
    ...
    execution_context = platform_riscv32_create(
...
static void elf_cleanup(tabos_app_context_t *context, int exit_status)
{
    (void)context;
    (void)exit_status;
    platform_riscv32_destroy(execution_context);
    execution_context = NULL;
    loader_elf_unload(&loaded_image);
    active_context = NULL;
    active_console = NULL;
    elf_exit_requested = false;
}
