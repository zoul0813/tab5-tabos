...
static const tabos_color_t demo_vga_palette[DEMO_VGA_COLOR_COUNT] = {
    TABOS_RGB565(0, 0, 0),     TABOS_RGB565(0, 0, 170),    TABOS_RGB565(0, 170, 0),    TABOS_RGB565(0, 170, 170),
    TABOS_RGB565(170, 0, 0),   TABOS_RGB565(170, 0, 170),  TABOS_RGB565(170, 85, 0),   TABOS_RGB565(170, 170, 170),
    TABOS_RGB565(85, 85, 85),  TABOS_RGB565(85, 85, 255),  TABOS_RGB565(85, 255, 85),  TABOS_RGB565(85, 255, 255),
    TABOS_RGB565(255, 85, 85), TABOS_RGB565(255, 85, 255), TABOS_RGB565(255, 255, 85), TABOS_RGB565(255, 255, 255),
};
...
#if DEMO_SCREEN_WIDTH < DEMO_PANEL_MARGIN * 2U || DEMO_SCREEN_HEIGHT < DEMO_PANEL_MARGIN * 2U
#error "Demo screen must fit the panel margins"
#endif

#if DEMO_PLAYER_INSET < 0 || DEMO_PLAYER_INSET + DEMO_PLAYER_INNER_SIZE > DEMO_PLAYER_SIZE
#error "Player inner rectangle must fit the player"
#endif

#if DEMO_SPRITE_OFFSET < 0 || DEMO_SPRITE_OFFSET + DEMO_SPRITE_DRAW_WIDTH > DEMO_PLAYER_SIZE || \
...
int main(void)
{
    ...
    for (uint32_t row = 0U; row < DEMO_SPRITE_HEIGHT; ++row) {
        for (uint32_t column = 0U; column < DEMO_SPRITE_WIDTH; ++column) {
            sprite[row * DEMO_SPRITE_WIDTH + column] =
                ((row / DEMO_SPRITE_CHECKER_SIZE) + (column / DEMO_SPRITE_CHECKER_SIZE)) % 2U == 0U ?
    ...
    while (running) {
        ...
        while (tabos_input_poll(&event)) {
            ...
            switch (event.key) {
                case TABOS_KEY_UP:
                    ...
                    (void) tabos_graphics_set_letterbox_color(&graphics, demo_vga_palette[palette_index]);
                    break;
                case TABOS_KEY_DOWN:
                    palette_index = (palette_index + DEMO_VGA_COLOR_COUNT - 1U) % DEMO_VGA_COLOR_COUNT;
                    (void) tabos_graphics_set_letterbox_color(&graphics, demo_vga_palette[palette_index]);
                    break;
                // EASD because Tab5 keyboards WASD isn't naturally spaced
                case TABOS_KEY_E: y -= DEMO_MOVE_STEP; break;
                case TABOS_KEY_S: y += DEMO_MOVE_STEP; break;
                case TABOS_KEY_A: x -= DEMO_MOVE_STEP; break;
                case TABOS_KEY_D: x += DEMO_MOVE_STEP; break;
                case TABOS_KEY_Q: running = false; break;
                case TABOS_KEY_R: rotation = (tabos_graphics_rotation_t) ((rotation + 1) % DEMO_ROTATION_COUNT); break;
                default: break;
            }
        }
        if (x < 0) {
        ...
        (void) tabos_graphics_fill_rect(&graphics, x, y, DEMO_PLAYER_SIZE, DEMO_PLAYER_SIZE, DEMO_COLOR_PLAYER);
        (void) tabos_graphics_fill_rect(&graphics, x + DEMO_PLAYER_INSET, y + DEMO_PLAYER_INSET, DEMO_PLAYER_INNER_SIZE,
