...
{
    const uint64_t fps_tenths = elapsed != 0U ? (uint64_t) frames * 10000U / elapsed : 0U;
    printf("%s: %u frames in %llu ms (%llu.%llu FPS)\n", name, frames, (unsigned long long) elapsed,
           (unsigned long long) (fps_tenths / 10U), (unsigned long long) (fps_tenths % 10U));
}
...
int main(void)
{
    ...
    for (unsigned int frame = 0U; frame < phase_frames; ++frame) {
        if (tabos_graphics_present(&graphics) != 0) {
            return 2;
        }
    }
    ...
    printf("Queue: %llu ms; present/drain: %llu ms; %s acceleration\n", (unsigned long long) queue_ms,
           (unsigned long long) present_ms,
           (capabilities & TABOS_GRAPHICS_CAP_HARDWARE_ACCELERATED) != 0U ? "hardware" : "software");
    return 0;
}
