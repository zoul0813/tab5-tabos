...
void tester_test_input(tester_context_t *context)
{
    ...
    if (original_flags < 0) return;

    tester_expect(context, fcntl(STDIN_FILENO, F_SETFL, original_flags | O_NONBLOCK) == 0,
                  "fcntl enables O_NONBLOCK");
    ...
    tester_expect(context, fcntl(STDIN_FILENO, F_SETFL, original_flags) == 0,
                  "fcntl restores stdin flags");
}
