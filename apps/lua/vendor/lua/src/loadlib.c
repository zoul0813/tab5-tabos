...
static void setpath (lua_State *L, const char *fieldname,
  ...
  (void)envname;
  (void)dft;
  lua_pushstring(L, strcmp(fieldname, "path") == 0 ? LUA_TABOS_PATH : "");
...
static int ll_loadlib (lua_State *L) {
  ...
  const char *init = lua_tabos_checkpath(L, 2);
...
static int ll_searchpath (lua_State *L) {
  const char *f = searchpath(L, lua_tabos_checkpath(L, 1),
                                lua_tabos_checkpath(L, 2),
                                lua_isnoneornil(L, 3) ? "." : lua_tabos_checkpath(L, 3),
                                lua_isnoneornil(L, 4) ? LUA_DIRSEP : lua_tabos_checkpath(L, 4));
  if (f != NULL) return 1;
...
static int searcher_Lua (lua_State *L) {
  const char *filename;
  const char *name = lua_tabos_checkpath(L, 1);
