...
static int io_open (lua_State *L) {
  const char *filename = lua_tabos_checkpath(L, 1);
  const char *mode = lua_isnoneornil(L, 2) ? "r" : lua_tabos_checkpath(L, 2);
  LStream *p = newfile(L);
...
static int io_tmpfile (lua_State *L) {
  return lua_tabos_unsupported(L);
}
...
static int g_iofile (lua_State *L, const char *f, const char *mode) {
  if (!lua_isnoneornil(L, 1)) {
    ...
    if (filename) {
      opencheck(L, lua_tabos_checkpath(L, 1), mode);
    }
    else {
...
static int g_read (lua_State *L, FILE *f, int first) {
  if (f == stdin) {
    return lua_tabos_console_read(L, first);
  }
