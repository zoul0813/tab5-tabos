...
int main(void)
{
    ...
    if (!filesystem_normalize_path("/apps//./bin/../hello", "A:/", normalized, sizeof(normalized)) ||
        strcmp(normalized, "A:/apps/hello") != 0 ||
        !filesystem_normalize_path("../../etc", "A:/home/user", normalized, sizeof(normalized)) ||
    ...
    if (!filesystem_init() || !filesystem_is_mounted()) {
        return 1;
    }
    tabos_drive_info_t drive;
    if (tabos_fs_drive_count() != 1U || !tabos_fs_drive_info(0U, &drive) || drive.letter != 'A' || !drive.mounted ||
        drive.name == NULL || tabos_fs_drive_info(1U, &drive)) {
        return 1;
    }
    ...
    if (tabos_fs_mkdir("/apps", 0755U) != 0 || tabos_fs_chdir("/apps") != 0) {
        return 1;
    }
    ...
    const tabos_fd_t descriptor = tabos_fs_open("hello.txt", TABOS_O_CREAT | TABOS_O_RDWR | TABOS_O_TRUNC, 0644U);
    static const char message[] = "Hello from TabOS filesystem";
    if (descriptor < 0 || tabos_fs_write(descriptor, message, sizeof(message)) != (tabos_ssize_t) sizeof(message) ||
    ...
    if (directory < 0 || !has_entry(directory, "renamed.txt") || tabos_fs_closedir(directory) != 0 ||
        tabos_fs_closedir(directory) == 0) {
        return 1;
    }

    char symlink_path[TABOS_FS_PATH_MAX];
    const int printed = snprintf(symlink_path, sizeof(symlink_path), "%s/apps/escape", test_storage_root());
    if (printed <= 0 || (size_t) printed >= sizeof(symlink_path) || symlink("/tmp", symlink_path) != 0 ||
        tabos_fs_stat("escape", &status) == 0 || *tabos_errno_location() != TABOS_EACCES || unlink(symlink_path) != 0) {
        return 1;
    }

    if (tabos_fs_unlink("renamed.txt") != 0 || tabos_fs_chdir("/") != 0 || tabos_fs_rmdir("/apps") != 0 ||
        tabos_fs_stat("/apps", &status) == 0 || *tabos_errno_location() != TABOS_ENOENT) {
        return 1;
    }
