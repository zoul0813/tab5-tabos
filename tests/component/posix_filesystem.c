...
int main(void)
{
    ...
    if (descriptor < 0 || write(descriptor, message, sizeof(message)) != (ssize_t) sizeof(message) ||
        lseek(descriptor, 0, SEEK_SET) != 0 || read(descriptor, buffer, sizeof(buffer)) != (ssize_t) sizeof(buffer) ||
    ...
    if (entry == NULL || strcmp(entry->d_name, "example.txt") != 0 || closedir(directory) != 0 ||
        unlink("/portable/example.txt") != 0 || rmdir("/portable") != 0) {
        return 1;
    }
    if (open("/missing", O_RDONLY) >= 0 || errno != ENOENT) {
        return 1;
    }
    filesystem_shutdown();
    if (open("/portable/example.txt", O_RDONLY) >= 0 || errno != ENODEV) {
        return 1;
    }
    return 0;
}
