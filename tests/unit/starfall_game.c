...
int main(void)
{
    ...
    if (game.mode != STARFALL_TITLE || game.high_score != 900U) {
        return 1;
    }
    starfall_game_start(&game);
    if (game.mode != STARFALL_PLAYING || game.lives != 3U || game.wave != 1U) {
        return 1;
    }
    ...
    if (game.player_x <= start_x) {
        return 1;
    }
    game.input.right = false;
    game.input.fire  = true;
    for (unsigned int tick = 0U; tick < 9U; ++tick) {
        starfall_game_update(&game);
    }
    ...
    starfall_game_update(&game);
    if (game.lives != 2U || game.invulnerable_until <= game.tick) {
        return 1;
    }
    game.enemies[0] = (starfall_enemy_t) {
        .x      = game.player_x,
    ...
    if (game.mode != STARFALL_PAUSED) {
        return 1;
    }
    const uint32_t paused_tick = game.tick;
    starfall_game_update(&game);
    if (game.tick != paused_tick) {
        return 1;
    }
    starfall_game_toggle_pause(&game);
    ...
    starfall_game_start(&game);
    return game.mode == STARFALL_PLAYING && game.lives == 3U && game.high_score == 1200U ? 0 : 1;
}
