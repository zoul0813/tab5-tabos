...
{
    upscale_valid = options != NULL && options->pixels != NULL &&
        options->bitmap_width == expected_width && options->bitmap_height == expected_height &&
        options->source.width == expected_width && options->source.height == expected_height &&
        options->destination.x == expected_x && options->destination.y == expected_y &&
        options->destination.width == expected_output_width &&
        options->destination.height == expected_output_height &&
...
int main(void)
{
    ...
    if (tabos_graphics_open(&invalid) != -1) return 1;
    if (tabos_graphics_open(&graphics) != 0 ||
        graphics.scale != 3U || graphics.output_x != 160U || graphics.output_y != 0U ||
        graphics.output_width != 960U || graphics.output_height != 720U ||
        graphics.letterbox_color != 0U ||
        tabos_graphics_set_letterbox_color(&graphics, red) != 0) return 1;
    expected_width = 320U; expected_height = 240U;
    expected_x = 160; expected_y = 0;
    expected_output_width = 960U; expected_output_height = 720U;
    if (tabos_graphics_present(&graphics) != 0 || clear_count != 1U ||
        cleared_color != red || !upscale_valid || tabos_graphics_close(&graphics) != 0 ||
        close_count != 1U) return 1;

    graphics = (tabos_graphics_t){.width = 320U, .height = 180U};
    if (tabos_graphics_open(&graphics) != 0 ||
        graphics.width != 320U || graphics.height != 180U || graphics.scale != 4U ||
        tabos_graphics_pixels(&graphics) == NULL ||
        (tabos_graphics_capabilities(&graphics) & TABOS_GRAPHICS_CAP_SCALED_CANVAS) == 0U) {
        return 1;
    }
    if (tabos_graphics_clear(&graphics, blue) != 0 ||
        tabos_graphics_fill_rect(&graphics, -2, -2, 4U, 4U, red) != 0 ||
        graphics.pixels[0] != red || graphics.pixels[1] != red ||
        graphics.pixels[2] != blue) return 1;

    const tabos_color_t sprite[] = {red, blue, blue, red};
    const tabos_graphics_blit_options_t blit = {
        .pixels = sprite, .bitmap_width = 2U, .bitmap_height = 2U,
        .source = {.width = 2U, .height = 2U},
        .destination = {.x = 10, .y = 10, .width = 4U, .height = 4U},
        .rotation = TABOS_GRAPHICS_ROTATE_90, .opacity = 255U,
    };
    expected_width = 320U; expected_height = 180U;
    expected_x = 0; expected_y = 0;
    expected_output_width = 1280U; expected_output_height = 720U;
    if (tabos_graphics_blit_ex(&graphics, &blit) != 0 ||
        tabos_graphics_present(&graphics) != 0 || !upscale_valid || present_count != 2U ||
        clear_count != 1U ||
        tabos_graphics_close(&graphics) != 0 || close_count != 2U || graphics.open) return 1;

    graphics = (tabos_graphics_t){0};
    if (tabos_graphics_open(&graphics) != 0 || graphics.width != 1280U ||
        graphics.height != 720U || graphics.scale != 1U || graphics.pixels != NULL ||
        tabos_graphics_close(&graphics) != 0 || close_count != 3U) return 1;
    return 0;
}
