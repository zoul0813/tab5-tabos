...
static void* connect_concurrently(void* argument)
{
    ...
    for (unsigned int index = 0U; index < CONCURRENT_OPERATION_COUNT; ++index) {
        ...
        /* Overlapping control transactions may be rejected without mutation. */
        (void) network_service_connect(ssid, "concurrent-password", false);
    }
    return NULL;
}
...
static void* disconnect_concurrently(void* argument)
{
    ...
    wait_for_concurrency_start(context);
    for (unsigned int index = 0U; index < CONCURRENT_OPERATION_COUNT; ++index) {
        /* The dedicated concurrency test checks rejection and shutdown drain. */
