...
static int expect_arguments(char *line, int expected_argc, const char *const *expected)
{
    ...
    if (argc != expected_argc) return 1;
    for (int index = 0; index < argc; ++index) {
        if (strcmp(argv[index], expected[index]) != 0) return 1;
    }
    return 0;
}
...
int main(void)
{
    ...
    return expect_arguments(plain, 3, plain_expected) ||
        expect_arguments(quoted, 3, quoted_expected) ||
