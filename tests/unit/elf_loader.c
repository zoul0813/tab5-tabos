...
int main(void)
{
    ...
    if (tab_elf_inspect(tab_hello_elf, tab_hello_elf_size, &info) != TAB_ELF_OK ||
        ...
        info.maximum_address != 85U || info.image_size != 85U ||
        ...
        !expect_mutation(68U, 86U, TAB_ELF_INVALID_SEGMENT) ||
        !expect_mutation(24U, 256U, TAB_ELF_INVALID_ENTRY) ||
        !expect_mutation(72U, (1024U * 1024U) + 1U, TAB_ELF_IMAGE_TOO_LARGE) ||
        !expect_mutation(232U, 9U, TAB_ELF_UNSUPPORTED_RELOCATION)) {
        return 1;
    }

    tab_elf_image_t image;
    if (tab_elf_load(tab_hello_elf, tab_hello_elf_size, &image) != TAB_ELF_OK ||
        image.memory == NULL || image.entry != image.memory || image.memory_size != 85U ||
        memcmp(image.memory, tab_hello_elf + 84U, 85U) != 0) {
        return 1;
    }
