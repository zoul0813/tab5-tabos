...
int tabos_graphics_open(tabos_graphics_t *graphics)
{
    ...
    *graphics = (tabos_graphics_t){
        ...
        .output_width = physical_width, .output_height = physical_height,
        .scale = 1U, .open = true,
    };
    if (requested_width == 0U) return 0;
    ...
    tabos_color_t *pixels = calloc((size_t)requested_width * requested_height,
                                   sizeof(*pixels));
    if (pixels == NULL) {
        (void)tabos_runtime_api->graphics_close();
        *graphics = (tabos_graphics_t){0};
...
int tabos_graphics_set_letterbox_color(tabos_graphics_t *graphics,
                                       tabos_color_t color)
{
    if (!valid(graphics) || !scaled(graphics)) { errno = EINVAL; return -1; }
...
int tabos_graphics_clear(tabos_graphics_t *graphics, tabos_color_t color)
{
    ...
    if (scaled(graphics)) {
        ...
        for (size_t index = 0U; index < count; ++index) graphics->pixels[index] = color;
        return 0;
    }
                             ...
                             uint32_t width, uint32_t height, tabos_color_t color)
{
    ...
    if (scaled(graphics)) {
        ...
        const int32_t clipped_right = right > (int64_t)graphics->width
            ? (int32_t)graphics->width : (int32_t)right;
        const int32_t clipped_bottom = bottom > (int64_t)graphics->height
            ? (int32_t)graphics->height : (int32_t)bottom;
        for (int32_t row = top; row < clipped_bottom; ++row) {
            tabos_color_t *destination = graphics->pixels +
                (size_t)row * graphics->width + (size_t)left;
                        ...
                        uint32_t width, uint32_t height, const tabos_color_t *pixels)
{
    ...
    if (scaled(graphics)) {
        const tabos_graphics_blit_options_t options = {
            .pixels = pixels, .bitmap_width = width, .bitmap_height = height,
...
uint32_t tabos_graphics_capabilities(const tabos_graphics_t *graphics)
{
    ...
    if (scaled(graphics)) capabilities |= TABOS_GRAPHICS_CAP_SCALED_CANVAS;
    return capabilities;
}

static int scaled_blit(tabos_graphics_t *graphics,
                       const tabos_graphics_blit_options_t *options)
{
    if (options->bitmap_width == 0U || options->bitmap_height == 0U ||
    ...
    for (uint32_t dy = 0U; dy < options->destination.height; ++dy) {
        ...
        if (output_y < 0 || output_y >= (int64_t)graphics->height) continue;
        ...
        for (uint32_t dx = 0U; dx < options->destination.width; ++dx) {
            ...
            if (output_x < 0 || output_x >= (int64_t)graphics->width) continue;
            ...
            const tabos_color_t source = options->pixels[
                (size_t)sy * options->bitmap_width + sx];
            if (options->color_key_enabled &&
                keyed(source, options->color_key_low, options->color_key_high)) continue;
            tabos_color_t *destination = &graphics->pixels[
                (size_t)output_y * graphics->width + (size_t)output_x];
                           ...
                           const tabos_graphics_blit_options_t *options)
{
    ...
    if (scaled(graphics)) return scaled_blit(graphics, options);
...
int tabos_graphics_present(tabos_graphics_t *graphics)
{
    ...
    if (scaled(graphics)) {
        if (tabos_runtime_api->graphics_blit_ex == NULL) { errno = ENOSYS; return -1; }
        const bool letterboxed = graphics->output_width != graphics->physical_width ||
            graphics->output_height != graphics->physical_height;
        if (letterboxed) {
            if (tabos_runtime_api->graphics_clear == NULL) { errno = ENOSYS; return -1; }
        ...
        if (result(tabos_runtime_api->graphics_blit_ex(&options)) != 0) return -1;
    }
