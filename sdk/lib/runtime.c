...
{
    return tabos_runtime_api != NULL && tabos_runtime_api->monotonic_ms != NULL ? tabos_runtime_api->monotonic_ms() :
                                                                                  0U;
}
...
{
    if (timer == NULL || !timer->active || tabos_monotonic_ms() < timer->deadline_ms) {
        return false;
    }
    if (timer->interval_ms == 0U) {
        timer->active = false;
    } else {
        timer->deadline_ms = tabos_monotonic_ms() + timer->interval_ms;
    }
    return true;
}
...
int tabos_system_info(tabos_system_info_t* info)
{
    ...
    info->memory_total_bytes = (uint64_t) source.memory_total_low | (uint64_t) source.memory_total_high << 32U;
    info->external_memory_total_bytes =
        (uint64_t) source.external_memory_total_low | (uint64_t) source.external_memory_total_high << 32U;
    return 0;
}
