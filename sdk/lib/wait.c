...
tabos_wait_source_t tabos_pointer_wait_source(tabos_pointer_stream_t stream)
{
    if (tabos_runtime_api == NULL || tabos_runtime_api->pointer_wait_source == NULL) {
        errno = ENOSYS;
        return TABOS_WAIT_SOURCE_INVALID;
    }
    const int result = tabos_runtime_api->pointer_wait_source(stream);
    if (result < 0) {
        errno = -result;
        return TABOS_WAIT_SOURCE_INVALID;
    }
